...
export class GeminiProvider extends Provider {
  ...
  /**
   * Normalizes model name by removing 'models/' prefix if present
   */
  ...
  /**
   * Resolves the model name from config
   */
  resolveModelName(config: ProviderConfig): string {
  ...
  async sendMessage(messages: Message[], config: ProviderConfig): Promise<string> {
    try {
      ...
      const tryParseJSON = (str: string): { valid: boolean; json?: string } => {
        try {
  ...
  override async validateImage(_base64Data: string, mimeType: string): Promise<ImageValidationResult> {
  ...
  override async getAvailableModels(config: ProviderConfig): Promise<string[]> {
    ...
    try {
      ...
      const models = (response.data?.models || [])
        ...
        .map((model: any) => this.normalizeModelName(model.name))
